...
class Tetirs:
    ...
    def __init__(self, height=20, width=10, block_size=20):
        ...
        self.extra_board = np.ones((self.height * self.block_size, 
                                    self.width * int(self.block_size / 2), 3),
    ...
    def reset(self):
        ...
        self.piece = np.array(self.pieces[self.ind])
        self.current_pos = {"x" : self.width // 2 - len(self.piece) // 2,
                            "y": 0}
        self.gameover = False
    ...
    def rotate(self, piece):
        return list(np.array(piece).transpose()[::-1])
    ...
    def get_state_properties(self, board):
        ...
        return torch.tensor([lines_cleared, holes, bumpiness, height], dtype=torch.float)
    ...
    def get_bumpiness_and_height(self, board):
        ...
        return total_height, total_bumpiness
    ...
    def render(self, speed=1, video=None):
        ...
        cv2.putText(img, "Score:", (self.width * self.block_size + int(self.block_size / 2), self.block_size), 
                    fontFace=1, fontScale=1.0, color=self.text_color)
        cv2.putText(img, str(self.score), (self.width * self.block_size + int(self.block_size / 2), self.block_size * 2), 
                    fontFace=1, fontScale=1.0, color=self.text_color)
        
        cv2.putText(img, "Pieces:", (self.width * self.block_size + int(self.block_size / 2), 
                                     self.block_size * 4), fontFace=1, fontScale=1.0, color=self.text_color)
        cv2.putText(img, str(self.tetrominoes), (self.width * self.block_size + int(self.block_size / 2), 
                                    self.block_size * 5), fontFace=1, fontScale=1.0, color=self.text_color)
        
        cv2.putText(img, "Lines:", (self.width * self.block_size + int(self.block_size / 2), 
                                    self.block_size * 7), fontFace=1, fontScale=1.0, color=self.text_color)
        cv2.putText(img, str(self.clear_line), (self.width * self.block_size + int(self.block_size / 2), 
                                    self.block_size * 8), fontFace=1, fontScale=1.0, color=self.text_color)
